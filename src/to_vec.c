...
/** Some ATARI stuff, not sure what this does. Caller must free() */
char *atari_gen_tempch(void) {
	char *tempch = NULL;
#ifdef ATARI
	int i;
	char *csname, *tempch;
	csname = calloc(strlen(p->outfile) + 5, sizeof(char));
	if (!csname) {
		printf("calloc err\n");
		return NULL;
	}
	tempch = calloc(strlen(p->outfile) + 5, sizeof(char));
	if (!tempch) {
		free(csname);
		printf("calloc err\n");
		return NULL;
...
void output_scalecmd(FILE *dest, const OUT_PAR * po, int mode) {
	char *atari_tempch;
	switch (mode) {
	case 0:		/* Metafont mode        */
		fprintf(dest, "mode_setup;\nbeginchar(\"Z\",%4.3fmm#,%4.3fmm#,0);\n",
				po->width, po->height);
		break;
	case 1:		/* TeX (em-Specials) mode       */
		fprintf(dest, "\\unitlength1mm\n\\begin{picture}(%4.3f,%4.3f)\n",
				po->width, po->height);
		break;
	...
	case 3:		/* TeXcad (\emline-Macros) mode */
		fprintf(dest, "\\unitlength=1mm\n\\begin{picture}(%4.3f,%4.3f)\n",
				po->width, po->height);
		break;
	case 4:		/* CS-Graphics specials for ATARI TeX */
		atari_tempch = atari_gen_tempch();
		if (atari_tempch == NULL) {
			break;
		}
		fprintf(dest, "\\unitlength1mm\n\\begin{draw}{%4.3f}{%4.3f}{%s}\n",
				po->width, po->height, atari_tempch);
		free(atari_tempch);
		break;
	case 5:		/* HP-GL mode */
		break;
...
/* mode should really be an enum. This is silly. */
int to_mftex(const GEN_PAR * pg, const OUT_PAR * po, int mode)
{
	...
	if (mode == 4) {
		...
		fprintf(csfile, "%% %s code in %s, created by hp2xx\n",
			ftype, tempch);
		output_scalecmd(csfile, po, mode);
